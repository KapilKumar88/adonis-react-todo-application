...
import { activityLogPaginationValidator } from '#validators/api/v1/admin/pagination'
...
export default class ActivityLogsController {
  ...
  async index({ request, response }: HttpContext) {
    const { page = 1, limit = 10, search, status, resource } = await request.validateUsing(activityLogPaginationValidator)
